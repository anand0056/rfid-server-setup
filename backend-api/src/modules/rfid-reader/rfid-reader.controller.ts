...
export class RfidReaderController {
  ...
  async getReaders(
    ...
    try {
      ...
      const readers = await this.rfidReaderService.getReaders(
        parsedTenantId,
        online === 'true',
      );
      return readers;
  ...
  async updateReader(@Param('id') id: string, @Body() updateData: any) {
    try {
      ...
      const reader = await this.rfidReaderService.updateReader(
        readerId,
        updateData,
      );
      if (!reader) {
