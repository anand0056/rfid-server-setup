...
import { parseTimezoneString } from 'src/common/utils/timezone';
...
export class RfidLogService {
  ...
  async getLogs({
      ...
      query.andWhere(
        new Brackets((qb) => {
          qb.where('log.card_uid LIKE :search', { search: `%${search}%` })
            .orWhere("COALESCE(staff.first_name, '') LIKE :search", {
              search: `%${search}%`,
            })
            .orWhere("COALESCE(staff.last_name, '') LIKE :search", {
              search: `%${search}%`,
            })
            .orWhere("COALESCE(vehicle.license_plate, '') LIKE :search", {
              search: `%${search}%`,
            })
            .orWhere("COALESCE(reader.name, '') LIKE :search", {
              search: `%${search}%`,
            })
    ...
    query.skip(offset).take(limit).orderBy('log.timestamp', 'DESC');
