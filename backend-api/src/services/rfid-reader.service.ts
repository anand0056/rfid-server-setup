...
export class RfidReaderService {
  ...
  async getReaders(tenantId?: number, onlineOnly?: boolean) {
    ...
    return query.orderBy('reader.tenant_id', 'ASC').addOrderBy('reader.name', 'ASC').getMany();
  }
  ...
  async createReader(reader: Partial<RfidReader>) {
    try {
      console.log('Service: Creating reader with data:', JSON.stringify(reader, null, 2));
      
      // If reader_group_id is provided, verify it exists
      if (reader.reader_group_id !== null && reader.reader_group_id !== undefined) {
        ...
        if (!groupExists) {
          console.error(`Reader group with ID ${reader.reader_group_id} does not exist`);
      ...
      console.log('Service: Created reader entity:', JSON.stringify(newReader, null, 2));
  ...
  async updateReader(id: number, update: Partial<RfidReader>) {
    try {
      // If reader_group_id is provided, verify it exists
      if (update.reader_group_id !== null && update.reader_group_id !== undefined) {
        const groupExists = await this.readerRepository.manager
          .getRepository('rfid_reader_groups')
          .createQueryBuilder('group')
        ...
        if (!groupExists) {
          console.error(`Reader group with ID ${update.reader_group_id} does not exist`);
          // Force it to undefined if it doesn't exist (will be set to NULL in DB)
