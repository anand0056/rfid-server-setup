export const parseTimezoneString = (dateStr: string): { date: Date; timezone: string } => {
  console.log('Parsing date string:', dateStr);
  const parts = dateStr.split(' ');
  if (parts.length !== 2) {
    throw new Error(`Invalid date string format. Expected "YYYY-MM-DD HH:mm:ss TIMEZONE", got "${dateStr}"`);
  }
  ...
  console.log('Parsed date info:', {
    ...
    explanation: cleanTimezone === 'IST' ? 'Converted IST to UTC by subtracting 5:30 hours' : 'No timezone conversion needed'
  });
